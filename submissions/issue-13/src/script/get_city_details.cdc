...
pub fun main(cityName: String): [NFT] {
    let arr :[NFT]=[]
    var cityids:[UInt64]=[]
    if TicketNFT.citySorted[cityName]==nil{
     ...
     while  i<keys.length {
        if TicketNFT.templateAddress.containsKey(keys[i]){
             ...
             let temp=getMixed(arr1: cityids, arr2: value)
           
             var j =0
              while j<temp.length {
                if get_detail_message(sellerAddress:keys[i],templateID:temp[j])!=nil{
                     arr.append(get_detail_message(sellerAddress:keys[i],templateID:temp[j])!)
                }
                j=j+1
              } 
        }
