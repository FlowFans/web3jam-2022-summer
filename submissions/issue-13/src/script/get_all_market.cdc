            ...
            let value=TicketNFT.templateAddress[keys[i]]!
             var j =0
              while j<value.length {
                if get_detail_message(sellerAddress:keys[i],templateID:value[j])!=nil{
                    arr.append(get_detail_message(sellerAddress:keys[i],templateID:value[j])!)
                }
                j=j+1
              } 
        }
