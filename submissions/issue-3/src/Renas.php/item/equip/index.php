...
$html->loadTpl('item/equip/body.frame.html', 'body');
...
//物品不可装备
if(is_null($item->occupancy['type'])) {
    \fLog("Error: item {$_GET['item']} is not equippable");
    $html->redirectBack(
        'redirect.message.item.transferError'
    );
    \fDie();
...
if($charCount > 0) {
    ...
    if($query !== false) {
        foreach ($query as $k => $charData) {
            ...
            elseif( //负载不足的角色不可接受物品
                $renderer->cc->total - $renderer->inventory->loading < $item->loads
            ) {
                $renderer->addCtrl(
                    "#",
                    'button.characterController.overloading',
                    'bgOpaGrey1 colorWhite1',
                    array('any'),
                    null,
                    true
            ...
            elseif( //没有空闲装备槽的角色不可接受物品
                $renderer->inventory->getAvailableSlots($item->occupancy['type']) < $item->occupancy['slots']
            ) {
                $renderer->addCtrl(
                    "#",
                    'button.characterController.insufficientEquipSlots',
                    'bgOpaGrey1 colorWhite1',
                    array('any'),
                    null,
                    true
