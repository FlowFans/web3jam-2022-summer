 ...
 * 
 * token
 * 通过这个选择器给什么物品赋予类型，token对应的是facility_stage中的数据记录
 * 
 * category
 * 是哪个父类下的物品类型
...
$html->loadTpl('editor/facility/dup.imageSelector.html', 'body');
...
if($stage === false) {
    \fLog("Error: no facility stage data fetched");
//还需要设计一个内容为空的页面
    \fDie();
} else {
    $stage = $stage[0];
...
$origData = \meshal\xFacility::getData($stage['name'], $stage['level']);
...
$html->set(
    ...
    (
        is_null($imageFile)
        || $imageFile == '' 
        || !file_exists(_ROOT.DIR_UPLOAD.$GLOBALS['deploy']['upload']['facilityImage'].$imageFile)
    ) 
        ? "{?!dirImg?}cardBg.default.facility.jpg" 
        : _ROOT.DIR_UPLOAD.$GLOBALS['deploy']['upload']['facilityImage'].$imageFile
);
