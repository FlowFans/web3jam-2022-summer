...
pub contract HunterGoodsItems: NonFungibleToken {
    ...
    // totalSupply
    // The total number of HunterGoodsItems that have been minted
    //
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        ...
        pub fun name(): String {
            return HunterGoodsItems.rarityToString(self.rarity)
                .concat(" ")
                .concat(HunterGoodsItems.kindToString(self.kind))
        }

        pub fun description(): String {
            return "A "
                .concat(HunterGoodsItems.rarityToString(self.rarity).toLower())
                .concat(" ")
                .concat(HunterGoodsItems.kindToString(self.kind).toLower())
                .concat(" with serial number ")
                .concat(self.id.toString())
        }

        pub fun imageCID(): String {
            return HunterGoodsItems.images[self.kind]![self.rarity]!
        }
    ...
    // This is the interface that users can cast their HunterGoodsItems Collection as
    // to allow others to deposit HunterGoodsItems into their Collection. It also allows for reading
    // the details of HunterGoodsItems in the Collection.
    pub resource interface HunterGoodsItemsCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowKittyItem(id: UInt64): &HunterGoodsItems.NFT? {
    ...
    //
    pub resource Collection: HunterGoodsItemsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @HunterGoodsItems.NFT
        ...
        pub fun borrowKittyItem(id: UInt64): &HunterGoodsItems.NFT? {
            if self.ownedNFTs[id] != nil {
                ...
                return ref as! &HunterGoodsItems.NFT
            } else {
                return nil
            }
        }
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver} {
            ...
            let kittyItem = nft as! &HunterGoodsItems.NFT
    ...
    pub resource NFTMinter {
        ...
        pub fun mintNFT(
            ...
            recipient.deposit(token: <-create HunterGoodsItems.NFT(id: HunterGoodsItems.totalSupply, kind: kind, rarity: rarity))

            emit Minted(
                id: HunterGoodsItems.totalSupply,
                kind: kind.rawValue,
                rarity: rarity.rawValue,
            )

            HunterGoodsItems.totalSupply = HunterGoodsItems.totalSupply + (1 as UInt64)
        }

        // Update NFT images for new type
        pub fun addNewImagesForKind(from: AuthAccount, newImages: {Kind: {Rarity: String}}) {
            let kindValue = HunterGoodsItems.images.containsKey(newImages.keys[0]) 
            if(!kindValue) {
                HunterGoodsItems.images.insert(key: newImages.keys[0], newImages.values[0])
    ...
    // If an account does not have a HunterGoodsItems.Collection, panic.
    // If it has a collection but does not contain the itemID, return nil.
    // If it has a collection and that collection contains the itemID, return a reference to that.
    //
    pub fun fetch(_ from: Address, itemID: UInt64): &HunterGoodsItems.NFT? {
        let collection = getAccount(from)
            .getCapability(HunterGoodsItems.CollectionPublicPath)!
            .borrow<&HunterGoodsItems.Collection{HunterGoodsItems.HunterGoodsItemsCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust HunterGoodsItems.Collection.borowKittyItem to get the correct itemID
    ...
    init() {
        ...
        // Set our named paths
        self.CollectionStoragePath = /storage/HunterGoodsItemsCollectionV14
        self.CollectionPublicPath = /public/HunterGoodsItemsCollectionV14
        self.MinterStoragePath = /storage/HunterGoodsItemsMinterV14
