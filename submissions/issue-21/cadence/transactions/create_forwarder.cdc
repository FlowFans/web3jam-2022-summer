...
transaction(receiver: Address) {
    ...
    prepare(acct: AuthAccount) {
        ...
        let recipient = getAccount(receiver)
            .getCapability<&{FungibleToken.Receiver}>(CarlyToken.ReceiverPublicPath)
        ...
        acct.save(<-vault, to: /storage/CarlyTokenForwarder)

        // Unlink the existing receiver capability
        if acct.getCapability(CarlyToken.ReceiverPublicPath).check<&{FungibleToken.Receiver}>() {
            acct.unlink(CarlyToken.ReceiverPublicPath)
        }

        // Link the new forwarding receiver capability
        acct.link<&{FungibleToken.Receiver}>(
            CarlyToken.ReceiverPublicPath,
            target: /storage/CarlyTokenForwarder
        )
    }
}