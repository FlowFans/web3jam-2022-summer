...
transaction(componentIdList: [UInt64]) {
    let freeMainCollectionRef: &SoulMadeMain.Collection
    let componentCollectionRef: &SoulMadeComponent.Collection

    prepare(acct: AuthAccount) {
        self.freeMainCollectionRef = acct.borrow<&SoulMadeMain.Collection>(from: /storage/SoulMadeMainCollectionFree)
    ...
    execute {
      for componentId in componentIdList{
        ...
        self.freeMainCollectionRef.deposit(token: <- newMain)
        if empty != nil{
          panic("Empty not nil")
        }
        destroy empty
